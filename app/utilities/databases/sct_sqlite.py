...
class DbBackEnd:
    ...
    def get_table_pk(self, table: str) -> dict:
        ...
        meta_dict["columns"] = [cd[1] for cd in curs.fetchall() if cd[5] == 1]
        return meta_dict
    ...
    def get_table_data(self, table: str, project_list: list, order_list: list, limit: int, offset: int = 0):
        ...
        """
        curs = self.get_cursor
        qry_str = SCT_QUERY_GET_PAGINATE_DATA.format(
            ",".join(project_list),
            table,
            ",".join(order_list),
            offset,
    ...
    def add_table_record(self, table: str, **kwargs):
        ...
        """
        # Query preparation
        table_columns = self.get_table_columns(table)["columns"]
        insert_columns = self.get_table_insert_columns(table)["columns"]
        qry_args = []
        for col in insert_columns:
            if (("int" in (table_columns[col]["type"]).lower() and
                 "point" not in (table_columns[col]["type"]).lower()) or
                    ("double" in (table_columns[col]["type"]).lower() or
                     "numeric" in (table_columns[col]["type"]).lower() or
                     "float" in (table_columns[col]["type"]).lower())):
    ...
    def drop_table_record(self, table: str, **kwargs):
        ...
        # Query preparation
        table_columns = self.get_table_columns(table)["columns"]
        pk_columns = self.get_table_pk(table)["columns"]
        qry_args = []
        if len(pk_columns):
